...
class PngStream(ChunkStream):
    ...
    def chunk_iTXt(self, pos: int, length: int) -> bytes:
        ...
        self.im_info[k_str] = self.im_text[k_str] = iTXt(
            v_str, lang_str, tk_str
        )
        self.check_text_memory(len(v_str))
    ...
    def _seek(self, frame: int, rewind: bool = False) -> None:
        ...
        if frame == 0:
            ...
            if not self.tile:
                ...
                raise EOFError(msg)
        if dispose_extent:
            self.dispose_extent: tuple[float, float, float, float] = (
                dispose_extent
            )
...
class _fdat:
    ...
    def __init__(
        self, fp: IO[bytes], chunk: Callable[..., None], seq_num: int
    ) -> None:
        self.fp = fp
        self.chunk = chunk
