...
class IFDRational(Rational):
    ...
    def __setstate__(
        self, state: list[float | Fraction | IntegralLike]
    ) -> None:
...
def _register_loader(
    idx: int, size: int
) -> Callable[[_LoaderFunc], _LoaderFunc]:
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _setup(self) -> None:
        ...
        logger.debug(
            "- YCbCr subsampling: %s", self.tag_v2.get(YCBCRSUBSAMPLING)
        )
        ...
        if self.use_load_libtiff:
            ...
            self.tile.append(
                ImageFile._Tile("libtiff", (0, 0, xsize, ysize), 0, a)
            )
...
class AppendingTiffWriter(io.BytesIO):
    ...
    def __init__(
        self, fn: StrOrBytesPath | IO[bytes], new: bool = False
    ) -> None:
        self.f: IO[bytes]
        if is_path(fn):
            self.name = fn
    ...
    @staticmethod
    def _verify_bytes_written(
        bytes_written: int | None, expected: int
    ) -> None:
