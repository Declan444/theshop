...
class ImagePointTransform:
    ...
    def __add__(
        self, other: ImagePointTransform | float
    ) -> ImagePointTransform:
...
def _getscaleoffset(
    ...
    return (
        (a.scale, a.offset) if isinstance(a, ImagePointTransform) else (0, a)
    )
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def __array_interface__(
        self,
    ) -> dict[str, str | bytes | int | tuple[int, ...]]:
    ...
    def getcolors(
        ...
        if self.mode in ("1", "L", "P"):
            ...
            out: list[tuple[int, float]] = [
                (h[i], i) for i in range(256) if h[i]
            ]
    ...
    def getxmp(self) -> dict[str, Any]:
        ...
        def get_value(element: Element) -> str | dict[str, Any] | None:
            value: dict[str, Any] = {
                get_name(k): v for k, v in element.attrib.items()
            }
    ...
    def remap_palette(
        self,
        dest_map: list[int],
        source_palette: bytes | bytearray | None = None,
    ) -> Image:
        """
    ...
    def resize(
        ...
        if reducing_gap is not None and resample != Resampling.NEAREST:
            ...
            if factor_x > 1 or factor_y > 1:
                reduce_box = self._get_safe_box(
                    size, cast(Resampling, resample), box
                )
    ...
    def thumbnail(
        ...
        def preserve_aspect_ratio() -> tuple[int, int] | None:
            def round_aspect(
                number: float, key: Callable[[int], float]
            ) -> int:
                return max(
                    ...
                    if tag_data[:8] == b"FUJIFILM":
                        ...
                        for i in range(
                            ...
                            if ifd_tag == 0x1101:
                                ...
                                self.fp.read(4)
                                camerainfo["InternalSerialNumber"] = (
                                    self.fp.read(4)
                                )
