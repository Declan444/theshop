...
class Countries(CountriesBase):
    ...
    def countries(self) -> Dict[str, Union[str, dict]]:
        ...
        if not hasattr(self, "_countries"):
            only: Iterable[Union[str, Tuple[str, str]]] = self.get_option(
                "only"
            )
            only_choices = True
            if only:
            ...
            if only and only_choices:
                ...
                override: Dict[str, Union[str, dict]] = self.get_option(
                    "override"
                )
                if override:
                ...
                for code, names in self.OLD_NAMES.items():
                    if code in self._countries and code not in override:
                        country_shadowed = self._shadowed_names.setdefault(
                            code, []
                        )
    ...
    def __iter__(self):
        ...
        ignore_first = (
            None if self.get_option("first_repeat") else self.countries_first
        )
        countries = tuple(
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            return list(
                itertools.islice(
                    self.__iter__(), index.start, index.stop, index.step
                )
            )
