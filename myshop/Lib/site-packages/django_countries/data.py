...
def self_generate(
    ...
    if not match:
        raise ValueError(
            'Expected a "COUNTRIES =" section in the source file!'
        )
    bits = match.groups()
    country_list = []
    ...
    if not alt_match:
        raise ValueError(
            'Expected an "ALT_CODES =" section in the source file!'
        )
...
def check_common_names():
    ...
    if common_names_missing:  # pragma: no cover
        print("")
        print(
            "The following common names do not match an official country code:"
        )
