...
class WhiteNoise(object):
    ...
    def __init__(self, application, root=None, prefix=None, **kwargs):
        ...
        if kwargs:
            raise TypeError(
                "Unexpected keyword argument '{0}'".format(
                    list(kwargs.keys())[0]
                )
            )
        ...
        if not callable(self.immutable_file_test):
            ...
            self.immutable_file_test = lambda path, url: bool(
                regex.search(url)
            )
        if root is not None:
    ...
    def add_file_to_dictionary(self, url, path, stat_cache=None):
        ...
        if self.index_file and url.endswith("/" + self.index_file):
            ...
            self.files[index_no_slash] = self.redirect(
                index_no_slash, index_url
            )
            url = index_url
    ...
    def add_cache_headers(self, headers, path, url):
        if self.immutable_file_test(path, url):
            ...
            headers["Cache-Control"] = "max-age={0}, public".format(
                self.max_age
            )
