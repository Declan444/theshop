...
class Reader(ResourceReader):
    ...
    def is_resource(self, path_):
        ...
        return any(
            len(parts) == 1 and parts[0] == path_
            for parts in map(part, self._contents)
        )
...
def create_package_from_loader(loader, is_package=True):
    ...
    spec = ModuleSpec(
        name, loader, origin="does-not-exist", is_package=is_package
    )
...
class CommonTests(metaclass=abc.ABCMeta):
    ...
    def test_useless_loader(self):
        package = create_package(
            file=FileNotFoundError(), path=FileNotFoundError()
        )
    ...
    def setUp(self):
        ...
        self.fixtures.enter_context(
            import_helper.DirsOnSysPath(
                str(zip_.make_zip_file(src_path, modules))
            )
        )
