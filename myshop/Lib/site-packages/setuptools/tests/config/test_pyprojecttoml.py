...
class TestEntryPoints:
    ...
    def pyproject(self, dynamic=None):
        project = {
            "dynamic": dynamic or ["scripts", "gui-scripts", "entry-points"]
        }
    ...
    def test_all_listed_in_dynamic(self, tmp_path):
        ...
        assert (
            expanded_project["entry-points"]["other"]["c"]
            == "mod.c:func [extra]"
        )
...
class TestClassifiers:
    ...
    def test_dynamic_without_config(self, tmp_path):
        ...
        with pytest.raises(
            OptionError, match="No configuration .* .classifiers."
        ):
    ...
    def test_dynamic_without_file(self, tmp_path):
        ...
        pyproject.write_text(cleandoc(config), encoding="utf-8")
        with pytest.warns(
            UserWarning, match="File .*classifiers.txt. cannot be found"
        ):
