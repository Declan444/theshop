...
from crispy_forms.tests.utils import (
    contains_partial,
    parse_expected,
    parse_form,
)
...
class TestBootstrapLayoutObjects:
    ...
    def test_prepended_appended_text(self, settings):
        ...
        if settings.CRISPY_TEMPLATE_PACK == "bootstrap":
            ...
            assert (
                dom.count(parse_html('<span class="add-on">gmail.com</span>'))
                == 1
            )
            assert dom.count(parse_html('<span class="add-on">#</span>')) == 1
            assert dom.count(parse_html('<span class="add-on">$</span>')) == 1
        ...
        if settings.CRISPY_TEMPLATE_PACK == "bootstrap3":
            ...
            test_form.helper.layout = Layout(
                PrependedAppendedText(
                    "email", "@", "gmail.com", css_class="input-lg"
                ),
            )
            html = render_crispy_form(test_form)
        ...
        if settings.CRISPY_TEMPLATE_PACK == "bootstrap4":
            ...
            assert (
                html.count('<span class="input-group-text">gmail.com</span>')
                == 1
            )
            assert html.count('<span class="input-group-text">#</span>') == 1
            assert html.count('<span class="input-group-text">$</span>') == 1
    ...
    def test_prepended_wrapper_class(self, settings):
        ...
        assert (
            html.count(
                '<div id="div_id_email" class="form-group wrapper class">'
            )
            == 1
        )
    ...
    def test_accordion_active_false_not_rendered(self, settings):
        ...
        assert (
            html.count(
                '<div id="one" class="%s collapse in"' % accordion_class
            )
            == 0
        )
