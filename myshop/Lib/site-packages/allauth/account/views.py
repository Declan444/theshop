...
class SignupByPasskeyView(SignupView):
    template_name = (
        "account/signup_by_passkey." + app_settings.TEMPLATE_EXTENSION
    )
...
class ConfirmEmailView(
    NextRedirectMixin, LogoutFunctionalityMixin, TemplateView
):
    ...
    def post(self, *args, **kwargs):
        ...
        email_address, response = (
            flows.email_verification.verify_email_and_resume(
    ...
    def get_ajax_data(self):
        ...
        if self.object:
            ...
            ret["user"] = {
                "display": user_display(self.object.email_address.user)
            }
        return ret
...
class EmailView(AjaxCapableProcessFormViewMixin, FormView):
        ...
        ret.update(
            {
                ...
                "can_add_email": EmailAddress.objects.can_add_email(
                    self.request.user
                ),
            }
        )
    ...
    def get_ajax_data(self):
        data = []
        for emailaddress in self.request.user.emailaddress_set.all().order_by(
            "pk"
        ):
            data.append(
                {
...
class PasswordChangeView(
    AjaxCapableProcessFormViewMixin, NextRedirectMixin, FormView
):
    template_name = (
...
class PasswordResetView(
    NextRedirectMixin, AjaxCapableProcessFormViewMixin, FormView
):
...
@method_decorator(
    rate_limit(action="reset_password_from_key"), name="dispatch"
)
@method_decorator(login_not_required, name="dispatch")
...
class LogoutView(NextRedirectMixin, LogoutFunctionalityMixin, TemplateView):
    ...
    def get_redirect_url(self):
        return self.get_next_url() or get_adapter(
            self.request
...
class ReauthenticateView(BaseReauthenticateView):
    ...
    def get_form_class(self):
        return get_form_class(
            app_settings.FORMS, "reauthenticate", self.form_class
        )
...
class RequestLoginCodeView(
    RedirectAuthenticatedUserMixin, NextRedirectMixin, FormView
):
    form_class = RequestLoginCodeForm
    template_name = (
        "account/request_login_code." + app_settings.TEMPLATE_EXTENSION
    )

    def get_form_class(self):
        return get_form_class(
            app_settings.FORMS, "request_login_code", self.form_class
        )
...
@method_decorator(
    login_stage_required(
        ...
        redirect_urlname="account_request_login_code",
    ),
    name="dispatch",
)
class ConfirmLoginCodeView(NextRedirectMixin, FormView):
    ...
    template_name = (
        "account/confirm_login_code." + app_settings.TEMPLATE_EXTENSION
    )
    ...
    def get_form_class(self):
        return get_form_class(
            app_settings.FORMS, "confirm_login_code", self.form_class
        )
