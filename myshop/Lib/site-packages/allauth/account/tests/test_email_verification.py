...
def test_email_verification_mandatory(
    settings, db, client, mailoutbox, enable_cache
):
    settings.ACCOUNT_EMAIL_CONFIRMATION_HMAC = False
...
def test_optional_email_verification(settings, client, db, mailoutbox):
    ...
    settings.ACCOUNT_EMAIL_VERIFICATION = (
        app_settings.EmailVerificationMethod.OPTIONAL
    )
...
def test_email_verification_hmac(
    settings, client, user_factory, mailoutbox, rf
):
    settings.ACCOUNT_EMAIL_CONFIRMATION_HMAC = True
...
def test_verify_logs_out_user(auth_client, settings, user, user_factory):
    ...
    auth_client.get(
        reverse(
            ...
            kwargs={"key": EmailConfirmationHMAC(email).key},
        )
    )
...
def test_email_verification_login_redirect(
    client, db, settings, password_factory
):
    settings.ACCOUNT_EMAIL_VERIFICATION = (
        app_settings.EmailVerificationMethod.MANDATORY
    )
