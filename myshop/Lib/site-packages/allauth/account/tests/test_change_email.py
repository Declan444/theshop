...
def test_change_email(user_factory, client, settings, mailoutbox):
    ...
    with patch(
        "allauth.account.signals.email_changed.send"
    ) as email_changed_mock:
...
def test_remove_email(
    ...
    assert EmailAddress.objects.filter(email=delete_email).exists() == (
        not success
    )
    if not success:
        assertTemplateUsed(
...
def test_dont_lookup_invalid_email(auth_client, email, did_look_up):
    with patch(
        "allauth.account.views.EmailAddress.objects.get_for_user"
    ) as gfu_mock:
