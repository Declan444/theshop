...
def request_login_by_code(mailoutbox):
    def f(client, email):
        ...
        resp = client.post(
            ...
            data={"email": email, "next": "/foo"},
        )
        assert resp.status_code == 302
        assert (
            resp["location"]
            == reverse("account_confirm_login_code") + "?next=%2Ffoo"
        )
        assert len(mailoutbox) == 1
        code = client.session[LOGIN_SESSION_KEY]["state"][
            LOGIN_CODE_STATE_KEY
        ]["code"]
        assert len(code) == 6
...
def test_login_by_code_max_attempts(
    client, user, request_login_by_code, settings
):
...
def test_login_by_code_unknown_user(mailoutbox, client, db):
    ...
    resp = client.post(
        reverse("account_confirm_login_code"), data={"code": "123456"}
    )
