...
class EmailAddressManager(models.Manager):
    ...
    def get_new(self, user):
        ...
        return (
            self.model.objects.filter(user=user, verified=False)
            .order_by("pk")
            .last()
        )
    ...
    def get_verified(self, user):
        return (
            self.filter(user=user, verified=True)
            .order_by("-primary", "pk")
            .first()
        )
    ...
    def get_users_for(self, email):
        ...
        return [
            address.user
            for address in self.filter(verified=True, email=email.lower())
        ]
...
class EmailConfirmationManager(models.Manager):
    ...
    def all_valid(self):
        return self.exclude(self.expired_q()).filter(
