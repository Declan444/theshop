...
def test_manage_providers(auth_client, user, headless_reverse, provider_id):
    ...
    resp = auth_client.delete(
        headless_reverse("headless:socialaccount:manage_providers"),
        data={
            "provider": account_to_del.provider,
...
def test_disconnect_bad_request(
    auth_client, user, headless_reverse, provider_id
):
    resp = auth_client.delete(
        headless_reverse("headless:socialaccount:manage_providers"),
    ...
    assert resp.json() == {
        "status": 400,
        "errors": [
            {"code": "account_not_found", "message": "Unknown account."}
        ],
    }
...
def test_auth_error_no_headless_request(
    client, db, google_provider_settings, settings
):
...
def test_provider_signup(client, headless_reverse, db, settings):
    ...
    pending_flow = [
        f for f in resp.json()["data"]["flows"] if f.get("is_pending")
    ][0]
...
def test_connect(
    user, auth_client, sociallogin_setup_state, headless_reverse, db
):
...
def test_connect_already_connected(
    ...
    assert resp.status_code == 302
    assert (
        resp["location"] == "/foo?error=connected_other&error_process=connect"
    )
