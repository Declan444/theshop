...
class ProviderTokenInput(inputs.Input):
    ...
    def clean(self):
        ...
        if provider_id and token:
            ...
            client_id_required = (
                provider_class is None or provider_class.uses_apps
            )
            client_id = token.get("client_id")
            if client_id_required and not isinstance(client_id, str):
                self.add_error(
                    "token", adapter.validation_error("client_id_required")
                )
            else:
                try:
                    ...
                    if not provider.supports_token_authentication:
                        ...
                        self.add_error(
                            "token",
                            adapter.validation_error("client_id_mismatch"),
                        )
                    else:
                        ...
                        if (
                            (
                                ...
                                and not isinstance(id_token, str)
                            )
                            or (
                            ...
                            self.add_error(
                                "token",
                                adapter.validation_error("token_required"),
                            )
        if not self.errors:
