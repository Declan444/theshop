...
def test_auth_unverified_email_and_mfa(
    ...
    user = user_factory(
        email_verified=False, password=password, with_totp=True
    )
    resp = client.post(
    ...
    data = resp.json()
    assert [f for f in data["data"]["flows"] if f["id"] == Flow.VERIFY_EMAIL][
        0
    ]["is_pending"]
...
def test_dangling_mfa_is_logged_out(
    ...
    data = resp.json()
    flow = [
        f for f in data["data"]["flows"] if f["id"] == Flow.MFA_AUTHENTICATE
    ][0]
