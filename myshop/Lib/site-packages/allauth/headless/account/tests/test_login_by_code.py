...
def test_login_by_code(headless_reverse, user, client, mailoutbox):
    ...
    data = resp.json()
    assert [f for f in data["data"]["flows"] if f["id"] == Flow.LOGIN_BY_CODE][
        0
    ]["is_pending"]
    assert len(mailoutbox) == 1
    code = [
        line for line in mailoutbox[0].body.splitlines() if len(line) == 6
    ][0]
    resp = client.post(
...
def test_login_by_code_max_attemps(headless_reverse, user, client, settings):
    ...
    for i in range(3):
        ...
        pending_flows = [
            f
            for f in session_resp.json()["data"]["flows"]
            if f.get("is_pending")
        ]
        if i >= 1:
