...
def test_acs(
    ...
    provider_settings = settings.SOCIALACCOUNT_PROVIDERS["saml"]["APPS"][0][
        "settings"
    ]
    ...
    finish_url = reverse(
        "saml_finish_acs", kwargs={"organization_slug": "org"}
    )
...
def test_metadata(
    ...
    resp = client.get(
        reverse("saml_metadata", kwargs={"organization_slug": "org"})
    )
    assert resp.status_code == 200
