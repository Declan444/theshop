...
def build_saml_config(request, provider_config, org):
    ...
    security_config = {
        ...
        "allowSingleLabelDomains": avd.get(
            "allow_single_label_domains", False
        ),
        "rejectDeprecatedAlgorithm": avd.get(
        ...
        "allowRepeatAttributeName": avd.get(
...
def decode_relay_state(relay_state):
    ...
    if relay_state:
        ...
        if (
            parts.scheme
            or parts.netloc
            or (parts.path and parts.path.startswith("/"))
        ):
