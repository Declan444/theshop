...
def email_address_clash(request_factory):
    def _email_address_clash(username, email):
        ...
        sociallogin = SocialLogin(
            user=user,
            account=account,
            email_addresses=[EmailAddress(email=email)],
        )
...
def test_email_address_created(
    ...
    assert SocialAccount.objects.filter(
        user=user, uid=sociallogin.account.uid
    ).exists()
...
def test_email_address_clash_username_not_required(
    db, settings, email_address_clash
):
...
def test_email_address_clash_username_auto_signup(
    db, settings, email_address_clash
):
    settings.ACCOUNT_EMAIL_REQUIRED = True
    settings.ACCOUNT_UNIQUE_EMAIL = True
...
def test_email_address_required_missing_from_sociallogin(
    ...
    resp = client.post(
        reverse("socialaccount_signup"), {"email": "other@example.org"}
    )
...
def test_email_address_conflict_removes_conflicting_email(
    ...
    # Here, we input the already existing email.
    resp = client.post(
        reverse("socialaccount_signup"), {"email": "other@email.org"}
    )
    assert (
        mailoutbox[0].subject
        == "[example.com] Please Confirm Your Email Address"
    )
