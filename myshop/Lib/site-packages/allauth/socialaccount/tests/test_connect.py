...
def test_disconnect(
    auth_client, user, settings, mailoutbox, reauthentication_required
):
    ...
    if reauthentication_required:
        ...
        assert (
            ...
            == "[example.com] Third-Party Account Disconnected"
        )
...
def test_connect(
    ...
    assert len(mailoutbox) == 1
    assert (
        mailoutbox[0].subject == "[example.com] Third-Party Account Connected"
    )
...
def test_connect_vs_email_authentication(
    ...
    with patch(
        ...
        assert (
            add_message.call_args[1]["message_context"]["action"]
            == expected_action
        )
