...
def test_email_authentication(
    ...
    if setting == "off":
        ...
        assert (
            SocialAccount.objects.filter(user=user.pk).exists() == auto_connect
        )
        assert (
            SocialToken.objects.filter(account__user=user.pk).exists()
            == auto_connect
        )
...
def test_record_authentication(
    ...
    sociallogin.token = SocialToken(
        app=sociallogin.account.get_provider().app,
        token="123",
        token_secret="456",
    )
