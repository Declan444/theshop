...
@method_decorator(
    login_stage_required(
        stage=AuthenticateStage.key, redirect_urlname="account_login"
    ),
    name="dispatch",
)
class AuthenticateView(TemplateView):
    ...
    def _build_forms(self):
        ...
        if self.request.method == "POST":
            if "code" in self.request.POST:
                ...
                self.auth_form = (
                    AuthenticateFormClass(user=user)
                    if support_webauthn
                    else None
                )
                posted_form = self.webauthn_form = (
    ...
    def get_form_class(self):
        return get_form_class(
            app_settings.FORMS, "authenticate", self.form_class
        )
