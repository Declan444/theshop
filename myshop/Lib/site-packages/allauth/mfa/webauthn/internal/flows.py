...
def signup_authenticator(
    request, user, name: str, credential: dict
) -> Authenticator:
...
def _signup_or_add_authenticator(
    ...
    adapter.add_message(
        request, messages.SUCCESS, "mfa/messages/webauthn_added.txt"
    )
    if not signup:
...
def remove_authenticators(
    request, authenticators: Iterable[Authenticator]
) -> None:
    raise_if_reauthentication_required(request)
...
def remove_authenticator(request, authenticator: Authenticator):
    ...
    adapter = get_account_adapter(request)
    adapter.add_message(
        request, messages.SUCCESS, "mfa/messages/webauthn_removed.txt"
    )
    adapter.send_notification_mail("mfa/email/webauthn_removed", request.user)


def perform_passwordless_login(
    request, authenticator: Authenticator, login: Login
):
