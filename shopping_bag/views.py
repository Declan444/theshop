    ...
    if request.user.is_authenticated:
        ...
        try:
            ...
            loyalty_points = LoyaltyPoints.objects.create(
                user=request.user, points=0
            )

    context = {
        "bag": bag,
        "loyalty_points": (
            loyalty_points.points if loyalty_points else 0
...
def add_to_shopping_bag(request, item_id):
    ...
    if size:
        if str(item_id) in bag:
            if size in bag[str(item_id)].get("items_by_size", {}):
                ...
                messages.success(
                    ...
                    f'{bag[str(item_id)]["items_by_size"][size]}'
                )
            else:
                bag[str(item_id)]["items_by_size"][size] = quantity
                messages.success(
        ...
        if str(item_id) in bag:
            ...
            messages.success(
                request,
                f"Updated {product.name} quantity to {bag[str(item_id)]}",
            )
        else:
            bag[str(item_id)] = quantity
            messages.success(
...
def remove_from_shopping_bag(request, item_id):
    ...
    try:
        ...
        if str(item_id) in bag:
            ...
            return JsonResponse(
                {"error": "Item not found in the bag"}, status=400
            )
